...
function runChat() {
    ...
    io.on("connection", function (client) {
        ...
        client.on("transfer", async function (msg) {
            ...
            transferHbar(msg.destinationId);
            balanceInTinyBar = await getBalanceInTinybar(process.env.ACCOUNT_ID);
...
async function getBalanceInTinybar(accountId) {
    const cookies =  AccountId.fromString(accountId);
    const balance = await new AccountBalanceQuery()
        .setAccountId(cookies)
        .execute(hederaClient);
...
async function transferHbar(destinationAccountId) {
    //Create the transfer transaction
    const cookies =  AccountId.fromString(process.env.ACCOUNT_ID);
    const jar = AccountId.fromString(destinationAccountId);
    const amountInTinybars = 22020222309;
    const sendHbar = await new TransferTransaction()
        .addHbarTransfer(cookies, Hbar.fromTinybars(amountInTinybars * -1)) //Sending account
        .addHbarTransfer(jar, Hbar.fromTinybars(amountInTinybars)) //Receiving account
        .execute(hederaClient);
